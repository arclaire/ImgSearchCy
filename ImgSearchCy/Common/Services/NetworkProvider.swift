        ...
        return Observable<Any?>.create({ (observer) -> Disposable in
            if Connectivity.isConnectedToInternet == false {
                ...
                do {
                    ...
                    header.add(HTTPHeader(name: "user-key", value: NetworkConfigurations.APIKeyZomato))
                    var urlRequest = try! URLRequest(url: URL(string: urlString)!, method: method, headers: header)//URLRequest(url: URL(string: urlString)!)
